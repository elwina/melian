...
func ExprASTResult(expr ExprAST) (float64, error) {
	var err error
	defer func() {
		if e := recover(); e != nil {
			err = e.(error)
	...
	case BinaryExprAST:
		...
		l, err = ExprASTResult(ast.Lhs)
		if err != nil {
			return 0.0, err
		}
		r, err = ExprASTResult(ast.Rhs)
		if err != nil {
			return 0.0, err
		}
		switch ast.Op {
		...
		case "/":
			if r == 0 {
				return 0.0, errors.New("violation of arithmetic specification: a division by zero in ExprASTResult")
				panic(errors.New(
		...
		case "%":
			...
			return float64(int(l) % int(r)), err
		case "^":
			return Pow(l, r), err
		default:
	...
	case NumberExprAST:
		return expr.(NumberExprAST).Val, err
	case FunCallerExprAST:
		f := expr.(FunCallerExprAST)
		def := defFunc[f.Name]
		return def.fun(f.Arg...), err
	}
